...
| `status` | text | `searching` / `matched` / `completed` / `cancelled` / `escaped`(撲空) |
...
> ⚠️ **後台不要呼叫 App 的 RPC**（`settle_escaped` / `submit_rating`），那兩個函式內部綁
> `auth.uid()`，後台的 service role 沒有 user 身分會驗證失敗。後台一律用 service role
> **直接 `update` orders / profiles**（要補錢包就直接改 `wallet_balance`）。
...
orders: id uuid, client_id uuid, hunter_id uuid, target_size text(小/大/飛),
  status text(searching|matched|completed|cancelled|escaped), price int,
  gender_pref text(any|male|female), min_completed int, needs_tools bool,
  is_vip bool, location_lat/lng float8, hunter_lat/lng float8, created_at timestamptz
...
profiles: id uuid(=auth.users.id), display_name text, avatar_url text, rating numeric,
  ...
  wallet_balance int, updated_at timestamptz
...
- 介面用繁體中文，沿用「那個，出現了！」品牌名；避免在任何文案出現「蟑」字（用「目標」）。
