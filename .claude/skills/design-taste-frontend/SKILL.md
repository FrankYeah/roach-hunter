...
> ## ⚠️ RN 適配備註（本專案專用，務必先讀）
>
> **本專案為 React Native（搭配 Expo / NativeWind），不是網頁。**
>
> 套用本 skill 的設計原則（先讀需求定方向、反 AI 預設樣板、間距／字級／色彩／動態紀律）時：
>
> - **一律輸出 React Native 寫法**：`StyleSheet.create` 或 **NativeWind `className`**。
> - **禁止**輸出任何 HTML 標籤（`<div>`、`<section>`、`<button>`…）與網頁 CSS（`:hover`、`box-shadow` 字串、media query、grid/flex 的 CSS 寫法請改用 RN 的 `flexDirection` / `gap` 等）。
> - 動態效果改用 RN 對應方案：`Animated` / `react-native-reanimated` / `Pressable` 的 `pressed` 狀態，而非 CSS transition / keyframes。
> - 陰影用 RN 的 `shadow*` / `elevation`，圓角、間距用 RN 數值或 Tailwind token。
>
> **適用範圍**：本 skill 最適用於 **行銷頁 / onboarding / 登入 / 啟動引導等視覺導向畫面**；
> 一般功能型畫面（表單、列表、設定）則**只取其設計紀律原則**（間距、字級階層、色彩克制、動態節制），不必追求高 VARIANCE 的視覺表現。
>
> 以下原文為網頁導向，請在心中將所有 HTML/CSS 範例「翻譯」成上述 RN 寫法後再產出。
